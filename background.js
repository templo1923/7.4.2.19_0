...
const w = {
  ...
  painel_Gestor: "https://miquecrm.com",
...
function A() {
  chrome.runtime.setUninstallURL(`https://miquecrm.com/api/urls/uninstalURL?id=${chrome.runtime.id}`);
}
function _(e) {
  e.reason === "install" && fetch(`https://miquecrm.com/api/urls/install?id=${chrome.runtime.id}`).then((t) => {
    if (!t.ok)
